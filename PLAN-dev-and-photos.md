...
**Подписка (централизованно, через админресурс):**

```
Вариант 1: QR-код на ресепшене при заселении
Вариант 2: Ссылка в Guest Portal → /start → бот привязывает chat_id к vaishnava
Вариант 3: Организатор ретрита рассылает ссылку на бота в общий чат
```
...
- [ ] Создать бота (@rupaseva_bot или аналог)
- [ ] Edge Function: отправка сообщений через Bot API
- [ ] Привязка chat_id к vaishnava (миграция: поле telegram_chat_id)
- [ ] Подписка: /start → бот привязывает chat_id к профилю
- [ ] Уведомление «Новые фото с ретрита!»
- [ ] Уведомление «Нашли вас на N фото!»
- [ ] Ссылка на бота в Guest Portal
