...
**Версия**: 1.5 (упрощение MVP: убраны прогрессбары, переиндексация, группировка по дням; авто-сжатие без подтверждения)
**Дата**: 2026-02-08
...
- Автосжатие крупных файлов:
  ...
  - Сжатие происходит автоматически, без подтверждения
  - Если после сжатия файл всё ещё > 15 МБ — файл отклоняется с пояснением: «{name} слишком большой. Обработайте вручную»
...
- [ ] Admin: upload.html — drag & drop, авто-сжатие > 15 МБ (без подтверждения), последовательная загрузка с retry
...
| 1.5 | upload.html: drag&drop + валидация форматов + авто-сжатие >15МБ + sequential upload + retry 3x + метаданные | Frontend | 5.0 | B | 3x | 1.67 | Canvas API resize стандартный, основное время — upload logic |
...
| | **Итого Фаза 1** | | **20.0** | | | **5.23** | |
| | **+ 15% overhead (ревью, интеграция)** | | | | | **6.0** | |

**Итого Фаза 1: ~20 часов (обычный dev, 7 дней) / ~6.0 часов (Human+Claude, 2 дня)**
...
| 3.2 | EF `telegram-webhook` (парсинг update, роутинг /start /stop) | Backend | 2.5 | B | 3.5x | 0.71 | Telegram Bot API хорошо документирован |
| 3.3 | Миграция: `telegram_chat_id` + `telegram_link_tokens` | DB | 1.0 | A | 6x | 0.17 | SQL-генерация |
| 3.4 | /start TOKEN (генерация, валидация TTL, привязка, идемпотентность, edge cases) | Backend | 3.5 | B | 3x | 1.17 | Сложный flow: token, expiry, re-linking |
| 3.5 | /stop отписка (SET telegram_chat_id = NULL) | Backend | 0.5 | A | 6x | 0.08 | Тривиально |
| 3.6 | EF `send-notification` (sendMessage, rate limit 30/сек, batch) | Backend | 2.0 | B | 3.5x | 0.57 | Telegram API + throttling |
...
| | **Итого Фаза 3** | | **15.5** | | | **5.52** | |
| | **+ 15% overhead** | | | | | **6.3** | |

**Итого Фаза 3: ~15.5 часов (обычный dev, 5-6 дней) / ~6.3 часов (Human+Claude, 2 дня)**
...
| | v1.3 | v1.4 | v1.5 (текущая) |
|--|------|------|----------------|
| Обычный dev | 35–47 ч | 61.5 ч | 52.5 ч |
| Human + Claude | 14.8 ч | 23.3 ч | **16.9 ч** |
| Дней (H+C, 3 ч/день) | ~5 | 7.5-8.5 | **~5.5** |
...
| Большие фото (>20 МБ, RAW) | Авто-сжатие может не хватить | Предупреждение «Обработайте вручную» |
...
|---|-----------|-------|-------------|
| 1 | Лимит файла 5 МБ, отклонение | **Авто-сжатие > 15 МБ на клиенте (без подтверждения)** | Камеры дают 7-15 МБ, некоторые 20+ МБ. Сжимаем автоматически, если не помогло — отклоняем |
